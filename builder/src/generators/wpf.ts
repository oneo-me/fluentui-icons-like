...
const RUNTIME_DIR = resolve(ROOT_DIR, 'packages', 'wpf', 'FluentUIIconsLike');
...
interface WpfIconRecord {
  symbol: string;
  variants: WpfIconVariantRecord[];
}

interface WpfIconVariantRecord {
  size: number;
  style: string;
  viewBox: string;
  pathData: string;
...
export const wpfGenerator: Generator = {
  name: 'wpf',
  generate(icons) {
...
function toIconRecords(icons: IconDefinition[]): WpfIconRecord[] {
  return icons
    .map((icon) => ({
...
function generateSymbolEnum(records: WpfIconRecord[]): string {
